...
var holiday;
var hasRequiredHoliday;
function requireHoliday() {
  if (hasRequiredHoliday) return holiday;
  hasRequiredHoliday = 1;
  var wdayNames = ["日", "月", "火", "水", "木", "金", "土"];
  var enHolidayNames = {
    "元日": "New Year's Day",
  ...
  holiday = function(date, name) {
    return {
      nameEn: enHolidayNames[name],
      wdayName: wdayNames[date.getDay()],
      date,
      name
    };
  };
...
var holidayJp;
var hasRequiredHolidayJp;
function requireHolidayJp() {
  if (hasRequiredHolidayJp) return holidayJp;
  hasRequiredHolidayJp = 1;
  var holidays = require$$0;
  var holiday2 = requireHoliday();
    ...
    between: function(start, end) {
      ...
      for (var key in holidays) {
        ...
        if (beginningOfDay(start) <= date && date <= beginningOfDay(end)) {
          res.push(holiday2(date, holidays[key]));
        }
      }
      return res;
    },
...
var holidayJpExports = requireHolidayJp();
const buildCalendarData = (currentDate) => {
  ...
  for (let day = 1; day <= lastDay; day++) {
    ...
    week.push({ date, isHoliday: holidayJpExports.isHoliday(date) });
    if (week.length === 7) {
      weeks.push(week);
      week = [];
    }
...
const Week = ({ week }) => {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { className: "calendar-body__row", children: week.map((calendar, index) => {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: classes, "data-key": key, children: calendar?.date.getDate() }, key);
  }) });
};
